...
public class TokenConduitPower extends Item
{
	...
	public ActionResult<ItemStack> onItemRightClick(World world, PlayerEntity player, Hand hand)
	{
		...
		if(!world.isRemote && player.isSneaking())
		{
			...
			player.sendStatusMessage(new TranslationTextComponent("item.powder_power.token_conduit_power.line2", EnableUtil.isEnabled(stack)).mergeStyle(TextFormatting.BOLD), true);
	...
	public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn)
	{
		...
		tooltip.add((new TranslationTextComponent("item.powder_power.token_conduit_power.line1").mergeStyle(TextFormatting.GREEN)));
		tooltip.add((new TranslationTextComponent("item.powder_power.token_conduit_power.line2", EnableUtil.isEnabled(stack)).mergeStyle(TextFormatting.RED)));
